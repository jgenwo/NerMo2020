...
// Semaphore for sensor value request on SWU
void CMouseCom::setRightBlock() {
    right_is_free = false;
}

// Semaphore for sensor value request on SWU
void CMouseCom::setLeftBlock() {
    left_is_free = false;
}
...
void CMouseCom::ProcessSpine(CMouseCom::typCmd cmd, int val1, int val2, int val3) {
    switch (cmd) {
        case SetMotorPos:
            ...
            break;
        case GetSensorValue:
            CMouseCom::sendSensorRequest(val1, val2); //takes ID and 0 for Knee / 1 for Foot / 2 for pos / 3 for PID
            break;
        case SetLed:
...
bool CMouseCom::sendSensorRequest(int id, int sensor) {
    //------ form String ------
    int l;
    char buffer[18];
    //String: ":ID?K"
    //sensor 0 --> Knee angle
    //String: ":ID?F"
    //sensor 1 --> Foot pressure
    //String: ":ID?P"
    //sensor 2 --> Servo Position
    //String: ":ID?C"
    //sensor 3 --> PID terms
    if (sensor == 0)
        l = sprintf(buffer, ":%02d?K\n", id);
    else if (sensor == 1)
        l = sprintf(buffer, ":%02d?F\n", id);
    else if (sensor == 2)
        l = sprintf(buffer, ":%02d?P\n", id);
    else if (sensor == 3)
        l = sprintf(buffer, ":%02d?C\n", id);
    //------ Send and Return ------
    return sendUartMessage(buffer, l);
}
...
bool CMouseCom::sendUartMessage(char buffer[], int l) {
    ...
    if (uart0_sendstream != -1) {
        int count = write(uart0_sendstream, &buffer[0],
                          l);        //Filestream, bytes to write, number of bytes to write
...
void CMouseCom::checkComndConsole() {
    ...
    if (tmp.valid) {
        if (DEBUG) {
            std::cout << "new Console Cmd read: " << (typCmd) tmp.command << ";" << tmp.val1 << ";" << tmp.val2 << ";"
                      << tmp.val3 << "\n";
        }
        //set false and store
        tmp.valid = false;
    ...
    if (uart0_readstream != -1) {
        ...
        while ((rx_length = read(uart0_readstream, (void *) rx_buffer, 255)) != 0) {
            ...
            if (rx_length < 0) {
                ...
                char *msg;
                char sensor;
                // to restore correct ID from message
                int index_offset, id;
                index_offset = 0;
                ...
                switch (arguments[0]) {
                    ...
                    case 'L':
                        ...
                        if (rx_length > 8) {
                            ...
                            id = atoi(&msg[5]) + index_offset;
                            sensor = msg[2];

                            switch (sensor) {
                                // parse knee sensor reply
                                case 'K':
                                    char val1[5], val2[5];
                                    unsigned int bx, by;
                                    ...
                                    bx = convertToInt(val1, 4);
                                    by = convertToInt(val2, 4);
                                    ReceiveMsg(KneeSensorValue, id, bx, by);

                                    break;
                                case 'F':
                                    // parse foot sensor reply
                                    char val[5];
                                    unsigned int value;
                                    ...
                                    value = convertToInt(val1, 4);
                                    ReceiveMsg(FootSensorValue, id, value);
                                    break;
                                case 'P':
                                    // parse position sensor reply
                                    char val3[5], val4[5];
                                    unsigned int current, desired;
                                    ...
                                    current = convertToInt(val3, 4);
                                    desired = convertToInt(val4, 4);

                                    ReceiveMsg(ServoSensorValue, id, current, desired);
                                    break;
                                case 'C':
                                    // parse PID reply
                                    char val5[5], val6[5], val7[5];
                                    unsigned int P, I, D;
                                    ...
                                    P = convertToInt(val5, 4);
                                    I = convertToInt(val6, 4);
                                    D = convertToInt(val7, 4);

                                    ReceiveMsg(PIDValues, id, P, I, D);
                                    break;
                            }
                            break;
                        }
...
// converts a char* in hex format to an unsigned integer
unsigned int CMouseCom::convertToInt(char *val, int length) {
    int i, temp;
    unsigned result;
    result = 0;
    for (i = 0; i < length; i++) {
        temp = convertHexToInt(val[i]);
        if (temp != -1)
            result += temp * pow(16, length - i - 1);
    }
    return result;
}

// helper method for convertToInt()
int CMouseCom::convertHexToInt(char digit) {
    switch (digit) {
        case '0':
            return 0;
        case '1':
...
void CMouseCom::ReceiveMsg(typCmd cmd, int val1, int val2, int val3, int val4) {
    int index, sensor;
    index = IDtoStoreArrayIndex(val1);
    switch (cmd) {
        case KneeSensorValue:
            if (sensor_index[index][0] >= storageBuffer)
                clearKneePart(index);
            ...
            //BX
            StoreArraySensor[index][0][sensor_index[index][0]] = val2;
            //BY
            StoreArraySensor[index][1][sensor_index[index][0]] = val3;

            //std::cout << val2 << " " << val3 << "\n";

            sensor_index[index][0] += 1;
            break;
        case FootSensorValue:
            if (sensor_index[index][1] >= storageBuffer)
                clearFootPart(index);

            StoreArraySensor[index][2][sensor_index[index][1]] = val2;

            //std::cout << val2 << "\n";

            sensor_index[index][1] += 1;
            break;

            /** case ServoSensorValue:
                int ID_Sensor = val1;
                int current = val2;
                int desired = val3;
                //TODO save in StoreArrayPos
                break;
            ...
            case PIDValues:
                ...
                int I = val3;
                int D = val3;
                //TODO save in StoreArrayPID
                break;*/
    }
}

void CMouseCom::clearSensorComplete() {
    for (int j = 0; j < 4; j++) {
        for (int i = 0; i < storageBuffer; i++) {
            for (int k = 0; k < 3; k++) {
                StoreArraySensor[j][k][i] = 0;
            }
        }
        sensor_index[j][0] = 0;
        sensor_index[j][1] = 0;
...
void CMouseCom::clearFootPart(int index) {
    for (int i = 0; i < storageBuffer; i++) {
        StoreArraySensor[index][2][i] = 0;
    }
    sensor_index[index][1] = 0;
}

void CMouseCom::clearKneePart(int index) {
    for (int i = 0; i < storageBuffer; i++) {
        StoreArraySensor[index][0][i] = 0;
        StoreArraySensor[index][1][i] = 0;
    }
    sensor_index[index][0] = 0;
}

int CMouseCom::IDtoStoreArrayIndex(int ID) {
    if (ID == 1)
        return 0;
    if (ID == 3)
        return 1;
...
void CMouseCom::setup_uart_read() {
    ...
    options.c_cflag = B1000000 | CS8 | CLOCAL | CREAD | CRTSCTS;;        //<Set baud rate
    options.c_iflag =
            IGNPAR | ICRNL; //ICRNL needed for putty and minicom else RPI will not recieve \n, do not know why!!!!
    options.c_oflag = 0;
    //options.c_lflag = 0;
    options.c_lflag = ICANON;
