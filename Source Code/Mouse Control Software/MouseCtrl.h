...
// init coil positions for closed loop control
static double lhl = 180, lfl = 180, rhl = 180, rfl = 180;

class CSpinePos {
public:
    CSpinePos() { spine = tail = 0; }

    CSpinePos(double s, double t) {
        spine = s;
        tail = t;
    }
...
public:
    CMouseLeg(char _leg, char _side, int _pawLift) {
        vx = vy = 0;
        leg = _leg;
        side = _side;
        pawLift = _pawLift;
...
#define ID_HEAD_TILT 24
    ...
    typedef enum Direction {
        Fwd, Bkwd, left, right, stop, stance
    } typDir;
    typDir dir = stop;
...
private:
    ...
    static const int FLift = 35; //height of the foot lift //TODO check if higher is possible 25 default
    static const int HLift = 30; //height of the foot lift //TODO check if higher is possible 20 default
    ...
    void PrintSensorKnee();

    void PrintSensorFoot();
