...
CLegPos CKinematics::ikforeleg(double fx, double fy, char side) {
    ...
    fx = fx -
         fpos_init[0][0];         //origin position of the leg (easier for trajectory planning)(relative to center point of trajectory circle)
    fy = fy - fpos_init[0][1];
    ...
    if (fx < 0) { ft1 = M_PI - atan(fy / fx) - asin(flb * sin(ftb) / sqrt(pow(fx, 2) + pow(fy, 2))); }
    else if (fx > 0) { ft1 = -atan(fy / fx) - asin(flb * sin(ftb) / sqrt(pow(fx, 2) + pow(fy, 2))); }
...
double CKinematics::foreKneeServoAngle(double ft1, double ft2, char side) {
    ...
    centerdist = sqrt(pow(fl1, 2) + pow(fltibia, 2) -
                      (2 * fl1 * fltibia * cos(ft2 * M_PI / 180))); //length from attachment point to hip centre
...
double CKinematics::hindKneeServoAngle(double ht1, double ht2, char side) {
    ...
    //calculate length of string from hip to attachment point at leg
    centerdist = sqrt(pow(hl1, 2) + pow(hltibia, 2) -
                      (2 * hl1 * hltibia * cos(ht2 * M_PI / 180)));//length from attachment point to hip centre
    ...
    if (side == 'l') {                                                                   //left side-----------------
        ...
        {
            return lhlCoilInit + (kneeservoangle(hlsi - hls)) * 180 / M_PI;
        }        //returns the final servo angle value in (degrees)
    } else //if (side == 'r')
    {                                                                   //right side-----------------
        ...
        {
            return rhlCoilInit - (kneeservoangle(hlsi - hls)) * 180 / M_PI;
        }        //returns the final servo angle value in (degrees)
    }
    /*else {
        return -1;
