...
void CMouseCtrl::Greeting() {
    ...
    std::cout << "Sensors:----------------------------\n";
    std::cout << "c: Request sensor values\n";
    std::cout << "z: Store sensor values to file\n";
    std::cout << "u: Print sensor values\n";
    std::cout << "Controlled:----------------------------\n";
    std::cout << "g: Initialize pose controlled (abort with j)\n";
}
    ...
    while (OK) {
        ...
        switch (state) {
            case 'i':   //initalize pose
                ...
                state = 'h';
                break;
            case 'g':   //initalize pose
                dir = stop;
                std::cout << "init controlled (abort with 'j')" << std::endl;
                initControlled();
                messages = 0;
                state = 'h';
                break;
            ...
            case 'k':   //playback motion data continous
                PlayFile();
                break;
            case 'c':   //request all 8 Sensor Values
                std::cout << "request sensor values" << "\n";
                requestSensorValues();
                counterValueRequest += 1; // to check packet loss
                messages = 0;
                state = 'v';
                break;
            case 'u':   //print all new Sensor Values
                PrintSensorKnee();
                PrintSensorFoot();
                std::cout << lhl << " " << lfl << " " << rhl << " " << rfl << "\n";
                std::cout << "expected counter (only for manual request with c): " << counterValueRequest
                          << "\n"; // to check packet loss
                messages = 0;
                state = 'v';
                break;
            case 'v': // don't change anything -> behave like before
                break;
            case 'z': // store sensor values to file
                StoreFileSensors();
                messages = 0;
                state = 'h';
                break;
...
void CMouseCtrl::initControlled() {
    //TODO fine tune for specific setup
    // legs wear through very fast... :/
    double goal[4] = {-8, 50, 8, -50};
    ...
    TrottArray[0][A_HINDLEFT_HIP] = tmpLeg.leg;
    TrottArray[0][A_HINDLEFT_KNEE] = tmpLeg.coil;
    tmpLeg = LHindRight.GetNext();
    TrottArray[0][A_HINDRIGHT_HIP] = tmpLeg.leg;
    TrottArray[0][A_HINDRIGHT_KNEE] = tmpLeg.coil;
    tmpLeg = LForeLeft.GetNext();
    TrottArray[0][A_FORELEFT_HIP] = tmpLeg.leg;
    TrottArray[0][A_FORELEFT_KNEE] = tmpLeg.coil;
    tmpLeg = LForeRight.GetNext();
    TrottArray[0][A_FORERIGHT_HIP] = tmpLeg.leg;
    TrottArray[0][A_FORERIGHT_KNEE] = tmpLeg.coil;
    TrottArray[0][A_SPINE] = tmpSpine.spine;
    TrottArray[0][A_TAIL] = tmpSpine.tail;
    TrottArray[0][A_SPINE_FLEX] = 180;
    TrottArray[0][A_HEAD_PAN] = uPosHeadPan;
    TrottArray[0][A_HEAD_TILT] = uPosHeadTilt;

    // publish normal initialisation
    Publish(1);

    // start p-control

    // K_P for every leg, as K_P adaptive
    double p[4] = {0.1, 0.1, 0.1, 0.1};
    // iterate over all 4 legs
    int k;
    // change = 1 if angle needs to be changed / 0 when done
    int change[4] = {1, 1, 1, 1};
    // check is 0 if all angles reached goal
    int check = change[0] || change[1] || change[2] || change[3];
    // for error message
    int reachable[4] = {0, 0, 0, 0};
    // for error message
    int moving[4] = {0, 0, 0, 0};
    // to prompt final state: nominal or error message
    int finalState[4] = {0, 0, 0, 0};
    // standard initPos as used in the above pre initalization
    const double initPos[4] = {TrottArray[0][A_HINDLEFT_KNEE], TrottArray[0][A_FORELEFT_KNEE],
                               TrottArray[0][A_HINDRIGHT_KNEE], TrottArray[0][A_FORERIGHT_KNEE]};
    // actual angle for the coil to be stored
    double pos[4] = {TrottArray[0][A_HINDLEFT_KNEE], TrottArray[0][A_FORELEFT_KNEE],
                     TrottArray[0][A_HINDRIGHT_KNEE], TrottArray[0][A_FORERIGHT_KNEE]};
    // old knee angle to check if angle changed
    double oldAngle[4] = {0, 0, 0, 0};
    // actual measured knee angle
    double actual[4];
    // goal[k] - actual[k]
    double angleError[4];
    // K_P * error
    ...
    // calculate error by normal initialisation
    for (k = 0; k < 4; k++) {
        // angle = atan2(by, bx) * 180 / pi
        actual[k] = atan2(StoreArraySensor[k][1][sensor_index[k][0] - 1],
                          StoreArraySensor[k][0][sensor_index[k][0] - 1]) * 57.2958;
        angleError[k] = goal[k] - actual[k];
    }

    // control loop / abort with j
    while (check && messages != 'j') {
        for (k = 0; k < 4; k++) {

            // check angle changed?
            // increase or decrease K_P
            if (abs(angleError[k]) >= 1.0 && reachable[k] < 2) {
                if (abs(actual[k] - oldAngle[k]) < 0.2) {
                    moving[k] += 1;
                    p[k] = 0.15;
                } else {
                    moving[k] = 0;
                ...
                // knee angle was not changed for 30 iterations --> error message
                if (moving[k] >= 30) {
                    correction[k] = 0;
                    change[k] = 0;
                    finalState[k] = 2;
        ...
        // add correction to position and change to 0 to 360 degrees
        for (k = 0; k < 4; k++) {
            pos[k] += correction[k];
            ...
            // if goal angle not reachable --> error message
            if (reachable[k] >= 2) {
                finalState[k] = 3;
                change[k] = 0;
            }
        ...
        check = change[0] || change[1] || change[2] || change[3];

        // set positions
        //TODO check if spine not correct maybe with foot pressure
        TrottArray[0][A_TIMESTAMP] = 0;
        tmpLeg = LHindLeft.GetNext();
        TrottArray[0][A_HINDLEFT_HIP] = tmpLeg.leg;
        TrottArray[0][A_HINDLEFT_KNEE] = pos[0];
        tmpLeg = LHindRight.GetNext();
        TrottArray[0][A_HINDRIGHT_HIP] = tmpLeg.leg;
        TrottArray[0][A_HINDRIGHT_KNEE] = pos[2];
        tmpLeg = LForeLeft.GetNext();
        TrottArray[0][A_FORELEFT_HIP] = tmpLeg.leg;
        TrottArray[0][A_FORELEFT_KNEE] = pos[1];
        tmpLeg = LForeRight.GetNext();
        TrottArray[0][A_FORERIGHT_HIP] = tmpLeg.leg;
        TrottArray[0][A_FORERIGHT_KNEE] = pos[3];
        TrottArray[0][A_SPINE] = tmpSpine.spine;
        TrottArray[0][A_TAIL] = tmpSpine.tail;
        TrottArray[0][A_SPINE_FLEX] = 180;
        TrottArray[0][A_HEAD_PAN] = uPosHeadPan;
    ...
    // prompt + storing positions for later use in Trot etc.
    std::cout << "------------------------------------" << std::endl;
    std::cout << "init successful!" << std::endl;
    std::cout << "k = 0: left hind; k = 1: left front, k = 2: right hind, k = 3: right front" << std::endl;
    for (k = 0; k < 4; k++) {
        switch (finalState[k]) {
            case 1:
                std::cout << k << ": init angle reached" << std::endl;
                break;
            case 2:
                std::cout << k
                          << ": ERROR; check motor, tendon and leg because angle is not changed or re-init"
                          << std::endl;
                break;
            case 3:
                std::cout << k
                          << ": ERROR; check leg and goal angle because goal angle could not be reached or re-init"
                          << std::endl;
                break;
            default:
    ...
    std::cout << lhl << " " << lfl << " " << rhl << " " << rfl << " " << std::endl;
    std::cout << "Positions stored" << std::endl;
    std::cout << "------------------------------------" << std::endl;
}

// requests and stores all 4 knee angle sensor and 4 foot pressure sensor values
void CMouseCtrl::requestSensorValues() {
    ...
    // block Semaphore and send request
    setLeftBlock();
    ProcessSpine(GetSensorValue, ID_FORELEFT_HIP, 0);

    // wait for Semaphore to be cleared (reply)
    while (counter < limit && !(right_is_free && left_is_free)) {
        usleep(sleep);
        counter += 1;
    }
    counter = 0;
    ...
    // block Semaphore and send request
    setRightBlock();
    ProcessSpine(GetSensorValue, ID_FORERIGHT_HIP, 0);
    ...
    // block Semaphore and send request
    setLeftBlock();
    ProcessSpine(GetSensorValue, ID_HINDLEFT_HIP, 0);
    ...
    // block Semaphore and send request
    setRightBlock();
    ProcessSpine(GetSensorValue, ID_HINDRIGHT_HIP, 0);
    ...
    // block Semaphore and send request
    setLeftBlock();
    ProcessSpine(GetSensorValue, ID_FORELEFT_HIP, 1);
    ...
    // block Semaphore and send request
    setRightBlock();
    ProcessSpine(GetSensorValue, ID_FORERIGHT_HIP, 1);
    ...
    // block Semaphore and send request
    setLeftBlock();
    ProcessSpine(GetSensorValue, ID_HINDLEFT_HIP, 1);
    ...
    // block Semaphore and send request
    setRightBlock();
    ProcessSpine(GetSensorValue, ID_HINDRIGHT_HIP, 1);
...
void CMouseCtrl::Publish(int length) //handle output of the array values for calculated lengths
{
    ...
    for (i = 0; i < length; i++) {
        ...
        if (DEBUG) {
            ...
            // FIXME request sensor only for getting sensor values to plot graphs
            requestSensorValues();
            SendMotorMsgs(i);
            usleep(CommandDelay); //send delay between points
        }
        if (messages == 's') {
            std::cout << "EStop in Publish!\n";
            return;
        } //break for EStop
        //Storage
...
{
    //TODO check if 2048/pi * angle in radians (from -pi to pi) + 2048 gives better results (see servo board code)
    double maxPos = 4095;
    //int minPos = 0;
    double maxDeg = 360;
    //int minDeg = 0;
...
//print the array values for sensors and calculated angles in degrees (atan2(by, bx)*180/pi)
void CMouseCtrl::PrintSensorKnee() {
    std::cout << "K 01 BX: " << StoreArraySensor[0][0][sensor_index[0][0] - 1] << " count: " << sensor_index[0][0]
              << "\n"
              << "K 01 BY: " << StoreArraySensor[0][1][sensor_index[0][0] - 1] << " count: " << sensor_index[0][0]
              << "\n"
              << "K 03 BX: " << StoreArraySensor[1][0][sensor_index[1][0] - 1] << " count: " << sensor_index[1][0]
              << "\n"
              << "K 03 BY: " << StoreArraySensor[1][1][sensor_index[1][0] - 1] << " count: " << sensor_index[1][0]
              << "\n"
              << "K 11 BX: " << StoreArraySensor[2][0][sensor_index[2][0] - 1] << " count: " << sensor_index[2][0]
              << "\n"
              << "K 11 BY: " << StoreArraySensor[2][1][sensor_index[2][0] - 1] << " count: " << sensor_index[2][0]
              << "\n"
              << "K 13 BX: " << StoreArraySensor[3][0][sensor_index[3][0] - 1] << " count: " << sensor_index[3][0]
              << "\n"
              << "K 13 BY: " << StoreArraySensor[3][1][sensor_index[3][0] - 1] << " count: " << sensor_index[3][0]
              << "\n"
              << "------------------------------------" << std::endl
              << "K 01 angle: "
              << atan2(StoreArraySensor[0][1][sensor_index[0][0] - 1], StoreArraySensor[0][0][sensor_index[0][0] - 1]) *
                 57.2958 << "\n"
              << "K 03 angle: "
              << atan2(StoreArraySensor[1][1][sensor_index[1][0] - 1], StoreArraySensor[1][0][sensor_index[1][0] - 1]) *
                 57.2958 << "\n"
              << "K 11 angle: "
              << atan2(StoreArraySensor[2][1][sensor_index[2][0] - 1], StoreArraySensor[2][0][sensor_index[2][0] - 1]) *
                 57.2958 << "\n"
              << "K 13 angle: "
              << atan2(StoreArraySensor[3][1][sensor_index[3][0] - 1], StoreArraySensor[3][0][sensor_index[3][0] - 1]) *
                 57.2958 << "\n"
              << "------------------------------------" << std::endl;
}

void CMouseCtrl::PrintSensorFoot() //print the array values for sensors
{
    std::cout << "F 01: " << StoreArraySensor[0][2][sensor_index[0][1] - 1] << " count: " << sensor_index[0][1] << "\n"
              << "F 03: " << StoreArraySensor[1][2][sensor_index[1][1] - 1] << " count: " << sensor_index[1][1] << "\n"
              << "F 11: " << StoreArraySensor[2][2][sensor_index[2][1] - 1] << " count: " << sensor_index[2][1] << "\n"
              << "F 13: " << StoreArraySensor[3][2][sensor_index[3][1] - 1] << " count: " << sensor_index[3][1] << "\n"
              << "------------------------------------" << std::endl;
}
...
void CMouseCtrl::StoreFileSensors() {
    std::ofstream file;
    int l;
    char buffer[20];

    //wirte new Filename
    l = sprintf(buffer, "SensorStorage_%d.txt", fileSensorNr);
    //open File
    file.open(&buffer[0]);

    if (file.is_open()) {
        for (int i = 0; i < storageBuffer; i++) {
            file << (TimeStamp[i] - StartTime) << ","
                 << StoreArraySensor[0][0][i] << ","
                 << StoreArraySensor[0][1][i] << ","
                 << StoreArraySensor[0][2][i] << ","
                 << StoreArraySensor[1][0][i] << ","
                 << StoreArraySensor[1][1][i] << ","
                 << StoreArraySensor[1][2][i] << ","
                 << StoreArraySensor[2][0][i] << ","
                 << StoreArraySensor[2][1][i] << ","
                 << StoreArraySensor[2][2][i] << ","
                 << StoreArraySensor[3][0][i] << ","
                 << StoreArraySensor[3][1][i] << ","
                 << StoreArraySensor[3][2][i] << "\n";
        }
        ...
        clearSensorComplete(); // also resets index to write next input
...
CLegPos CMouseLeg::NextWayPoint() {
    ...
    rhlCoilInit = rhl;
    rflCoilInit = rfl;
    std::cout << lhlCoilInit << " " << lflCoilInit << " " << rhlCoilInit << " " << rflCoilInit << " " << std::endl;
...
CLegPos CMouseLeg::SetPosition(CLegPos ang) {
    ...
    ptLeg.x += vx;
    ptLeg.y += vy;      // Vektor auf letzten Punkt addieren
